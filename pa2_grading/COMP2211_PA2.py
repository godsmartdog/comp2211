...
Original file is located at
    https://colab.research.google.com/drive/1vnlNm5y0dpUUgYC74irx7_QSn_ig1hFn
...
def remove_duplicates(df):
  # Task 1: Check whether there are duplicate rows in df and remove them.
  ###### TO DO ######
...
def clean_text(text):
  # Task 2: Follow the steps below to complete the function. You will need to use the re library of most sub-tasks
  ###### TO DO ######
  ...
  ###################
    ...
    # 3. Handle negations, i.e., "not happy" => "not_happy"
    text = re.sub(r'not\s+(\w+)', r'not_\1', text)

    # 4. Remove punctuation - ⭐ REMOVE APOSTROPHES TOO!
    text = re.sub(r'[^\w\s]', '', text)  # Changed from [^\w\s_] to [^\w\s]
...
**Hint:** You may want to take a look at the imported libraries above while working on the code cells below.
"""
...
def text_preprocessing_mlp(text):
    ...
    ###################
    # This line joins the tokens (a list of words) back into a sentence. You may rename the variable `token` if you know what you're doing.
    return ' '.join(tokens)
...
def tfidf_vectorization(df_col, max_features):
  ...
  # 2. Apply the object to the dataframe column and return the array
  X = tfidf_vectorizer.fit_transform(df_col).toarray()
  return X
...
Complete the following 2 tasks in the cell block below.
...
def get_mlp_model():
  ...
  ###################
    mlp_model = Sequential([
        ...
        Dense(32, activation='relu'),   # ⭐ Smaller first layer
        Dropout(0.4),

        Dense(16, activation='relu'),   # ⭐ Smaller second layer
        Dropout(0.3),

        Dense(8, activation='relu'),    # ⭐ Smaller third layer
        Dropout(0.2),
...
def tokenize_text(df_col, num_words=600, max_len=200):
  # Task 8: Tokenize the data and return the padded sequences.
  ###### TO DO ######
  ...
  # 4. Pad the sequences using post padding and truncate from the end (hint: check imported functions).
    ...
    # oov_token is used to represent words not in the vocabulary (Out Of Vocabulary)
    ...
    # 4. Pad the sequences using post padding and truncate from the end
...
def get_cnn_model(max_len):
   # Task 9: Compile and build your CNN model. Return the compiled Keras model.
    cnn_model = Sequential([
        ...
        Dense(100, activation='relu'),  # Balanced size
        Dropout(0.5),
...
if __name__ == '__main__':
  early_stop = tf.keras.callbacks.EarlyStopping(
      ...
      restore_best_weights=True,        # ⭐ KEY: Roll back to best weights
      mode='max'                        # We want to maximize accuracy
  )

  # Model checkpoint to save the best model
  checkpoint = tf.keras.callbacks.ModelCheckpoint(
      'best_cnn_model.keras',           # File to save the best model
      monitor='val_accuracy',           # Watch validation accuracy
  ...
  print("Training with callbacks to break through 80% barrier...")
  history = cnn_model.fit(
      X_train,
      y_train,
      epochs=20,                        # Allow more epochs (early stop will cut it)
      batch_size=64,
      validation_split=0.2,
      callbacks=[early_stop, checkpoint, reduce_lr],  # ⭐ All callbacks
      verbose=1
  )

  # Load the BEST model from training (not the final one)
  cnn_model = tf.keras.models.load_model('best_cnn_model.keras')
  loss, accuracy = cnn_model.evaluate(x=X_test, y=y_test, batch_size=32)
  print(f"loss: {loss:.3f}")
  print(f"accuracy: {accuracy:.3f}")
###################
